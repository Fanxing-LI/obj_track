...
class Test(TestBase):
    ...
    def draw(self, names=None):
        ...
        # Use torch.stack to properly combine tensors, then convert to numpy
        import torch as th
        ...
        # Get target positions (quietly)
        targets = self.env.target
        if hasattr(targets, 'cpu'):
            targets = targets.cpu().numpy()
        
        # Extract collision points over time (T, N, 3) and compute collision vectors
        ...
        # Normalize list to an array with NaNs where missing
        ...
        for pts in collision_points_list:
            if pts is None:
                ...
                # Handle shapes (N,3) or flat (3,) for single agent
                arr = np.array(pts)
        ...
        try:
            ...
            # Fallback: broadcast/trim to match absolute_positions time/env dims
        ...
        # Compute vectors from agent position to closest collision point
        ...
        cmap = plt.cm.get_cmap('tab20', num_envs)  # noqa: F841
        ...
        for agent_idx in range(num_envs):
            ...
            # Draw collision vectors at each timestep for this agent
            ...
            if np.any(valid_mask):
                ...
                # Three-tier color coding by distance
                # Red: < 1.0m (danger zone)
                ...
                # Light blue: >= 2.0m (safe zone, awareness vectors)
        ...
        # Add target positions for all agents
        ...
        # Only show if we have a display (avoid Qt errors on remote servers). Saving is handled by caller.
        try:
            plt.show()
        except Exception:
            pass
    ...
    def draw_debug(self, names=None, save_path: Optional[str] = None):
        ...
        if hasattr(self.env, 'collision_dis') and self.env.collision_dis is not None:
            ...
            if collision_distances.ndim == 1:
                ...
                if len(collision_distances) == len(t):
                    ...
                    # Scalar case - skip plotting quietly
                    pass
            else:
                ...
                for agent_idx in range(num_envs):
                    ax7.plot(t, collision_distances[:, agent_idx], 
                            label='Agent {}'.format(agent_idx), alpha=0.7)
        ...
        # Save debug figure (per-episode if save_path provided)
