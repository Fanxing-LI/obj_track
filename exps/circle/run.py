...
def parse_args():
    parser = argparse.ArgumentParser(description='Run circle experiments', add_help=False)
    ...
    parser.add_argument("--env", "-e", type=str, default="circle")
...
if __name__ == "__main__":
    ...
    if args.train:
        ...
        # CircleEnv has target_position attribute
        ...
        print(f"Desired radius: {test_env.desired_radius}m")
        print(f"Tangential speed: {test_env.tangential_speed}m/s")
        ...
        try:
            ...
            spawn_center_cfg = [1.2, 0.0, 1.5]  # Default near circle
        spawn_center = th.tensor([spawn_center_cfg], dtype=th.float32)

        # Target from env attribute (center of circle)
        if hasattr(test_env, 'target_position'):
        ...
        # Create a ring around target (circle points at desired radius)
        ring_points = []
        ring_radius = test_env.desired_radius  # Use actual desired radius
        for angle in np.linspace(0, 2*np.pi, 32):  # More points for smoother circle
