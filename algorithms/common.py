...
class SimpleRolloutBuffer:
    ...
    def add(self, obs, reward, action, next_obs, done, episode_done, value, disc_r=None, d_value=None):
        ...
        self.disc_r.append(disc_r)
        self.d_value.append(d_value)
    ...
    def compute_returns(self):
        ...
        if self.d_value[0] is not None:
            self.d_returns = compute_td_returns(
                r=self.disc_r,
                done=self.done,
                next_value=self.d_value,
                episode_done=self.episode_done,
                gamma=self.gamma
            )
        # self.flatten()
    ...
    def flatten(self):
        ...
        # if self.include_disc_r:
        self.disc_r = th.vstack(self.disc_r)

    def toTensor(self):
        self.reward = th.stack(self.reward)
        self.obs = TensorDict.stack(self.obs)
        self.action = th.stack(self.action)
        self.next_obs = TensorDict.stack(self.next_obs)
        self.done = th.stack(self.done)
        self.episode_done = th.stack(self.episode_done)
        self.returns = th.stack(self.returns)
        # if self.include_disc_r:
        if self.disc_r[0] is not None:
            self.disc_r = th.stack(self.disc_r)
        if self.d_value[0] is not None:
