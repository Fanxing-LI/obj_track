...
for v_i, v in enumerate(vs):
    for alg_i, alg in enumerate(algs):
        ...
        # 创建3D图形
        plot_with_error_band(axes[0, v_i], t, mean_ori[:, 0], std_ori[:, 0])
        plot_with_error_band(axes[1, v_i], t, mean_ori[:, 1], std_ori[:, 1])
        plot_with_error_band(axes[2, v_i], t, mean_distance-3, std_distance)
