...
class FlipEnv(DroneGymEnvsBase):
    ...
    def get_reward(self) -> th.Tensor:
        ...
        # Dense Orientation Reward during non-flip phases
        # X-axis should point mostly forward [1, 0, 0] when not flipping
        x_alignment = x_axis[:, 0]  # How much x-axis aligns with world x
        z_alignment = z_axis[:, 2]  # How much z-axis aligns with world z (upright)
        
        # Only apply orientation reward when not actively flipping
        is_flipping = th.abs(pitch_angular_vel) > 2.0  # rad/s threshold
        orientation_reward = th.where(
            is_flipping,
            th.ones_like(x_alignment),  # No orientation penalty during flip
            th.exp(-2.0 * (1 - z_alignment).abs())  # Penalize when not upright and not flipping
        )
