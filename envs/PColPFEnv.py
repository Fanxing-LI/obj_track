...
class PColPFEnv(DroneGymEnvsBase):
    ...
    def get_reward(self):
        ...
        r_col_dis = (2 - collision_dis).clamp(min=0.0, max=2.0) * -0.05
        collision_direction = self.collision_vector / (collision_dis.unsqueeze(1) + 1e-8)
        r_col_vel = ((self.velocity - 0) * collision_direction).sum(dim=1).detach() * -0.05
        ...
        # Penalize misalignment: 0 when perfect (align=1), -0.04 when opposite (align=-1)
        r_percep_aware = (align - 1) * scale * 0.05
        
        threshold = 0.4
        dist = (self.position - self.target).norm(dim=1)
        pos_scale = dist.clamp_min(threshold).detach()
        r_pos = dist / pos_scale * -0.05
        r_vel = (self.velocity - 0).norm(dim=1) * -0.002
        r_ang_vel = (self.angular_velocity - 0).norm(dim=1) * -0.01
        return r_pos + r_vel + r_ang_vel + r_col_dis  + r_percep_aware + r_col_vel
