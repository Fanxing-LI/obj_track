...
class ObjectTrackingEnv(DroneGymEnvsBase):
    ...
    def __init__(
        ...
        # target head position only
        self.observation_space["state"] = spaces.Box(
            shape=(13,), low=-th.inf, high=th.inf, dtype=np.float32)
    ...
    def get_reward(self, *args, **kwargs) -> th.Tensor:
        ...
        act_r = self._action[:, 1:].norm(dim=1).to(vel_r.device) * -0.003 + self._action[:, 2:3].norm(dim=1).to(
            vel_r.device) * -0.025
