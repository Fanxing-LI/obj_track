...
class EasyNavigationEnv(DroneGymEnvsBase):
    def __init__(
        self,
    ...
    def get_reward(self) -> th.Tensor:
        ...
        r_percep_aware = (align - 1) * scale * 0.01
        ...
        r_col_dis = (1 - collision_dis).clamp(min=0.0, max=1.0) * -0.06
        collision_direction = self.collision_vector / (collision_dis.unsqueeze(1) + 1e-8)
        r_col_vel = (th.relu((self.velocity - 0) * collision_direction).sum(dim=1) * th.relu(1 - collision_dis)).detach() * -0.04
...
if __name__ == "__main__":
    ...
    target = [15.0, 5.0, 1]
    scene_path = "VisFly/datasets/visfly-beta/configs/scenes/box15_wall_center_pillar"
