...
class OrdNavigationEnv(DroneGymEnvsBase):
    def __init__(
        self,
        ...
        # Default to a 16x16 depth sensor (mirroring HardNavigationEnv)
        sensor_kwargs = [
            {
    ...
    def get_reward(self) -> th.Tensor:
        ...
        r_col_dis = (1.5 - collision_dis).clamp(min=0.0, max=1.5) * -0.1
        collision_direction = self.collision_vector / (collision_dis.unsqueeze(1) + 1e-8)
        r_col_vel = (th.relu(((self.velocity - 0) * collision_direction).sum(dim=1)) * th.relu(1.5 - collision_dis)).detach() * -0.1
