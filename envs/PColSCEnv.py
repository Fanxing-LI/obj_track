...
class PColSCEnv(DroneGymEnvsBase):
    """Pillar Collision Environment with configurable collision detection (scene-only by default)"""
    def __init__(
            self,
        ...
        # Collision config is handled by VisFly core
    ...
    def get_reward(self):
        """Same reward function as PillarCollisionEnv for fair comparison.
        """
        # Keep Unchange: perception aware and pos reward
        ...
        r_percep_aware = align * scale * 0.02
        ...
        # Distances to closest obstacle
        collision_dis = self.collision_vector.norm(dim=1)
        collision_dir = self.collision_vector / (collision_dis.unsqueeze(1) + 1e-8)
        # print(collision_dis[0])
        # Approach speed toward obstacle (positive when moving toward obstacle)
        approach_speed = ((self.velocity - 0)* collision_dir).sum(dim=1).relu().detach()

        # Collision reward
        safe_dis = 1.5
        r_col_vel = -0.02 * approach_speed * th.relu(safe_dis - collision_dis) ** 2
        r_col_dis = -0.02 * th.clamp(safe_dis - collision_dis, min=0.0) ** 2
       
        r_vel = (self.velocity - 0).norm(dim=1) * -0.002
        r_ang_vel = (self.angular_velocity - 0).norm(dim=1) * -0.01
        reward = r_vel + r_ang_vel + r_pos + r_percep_aware + r_col_vel + r_col_dis
        ...
        "state_generator":
            {
                ...
                "kwargs": [{
                    ...
                    "orientation": {"mean": [0.0, 0.0, 1.7508], "half": [0, 0, 0]},
                }]
            }
    }
    target = [15.0, 12.0, 1]
    scene_path = "VisFly/datasets/visfly-beta/configs/scenes/box15_wall_empty"
    sensor_kwargs = [{
        "sensor_type": SensorType.DEPTH,
        "uuid": "depth",
        "resolution": [64, 64],
